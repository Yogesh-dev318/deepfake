  ...
  try {
    ...
    if (genResult.ok && genResult.response) {
      ...
      if (!reportText || !reportText.trim()) {
        usedFallback = true;
        reportText = localImageFallback(result);
      }
    } else {
      usedFallback = true;
    ...
    if (result.image_id) { page.drawText(`Image ID: ${result.image_id}`, { x: margin, y: y - 11, size: 11, font }); y -= 11 + 10; }
    ...
    const renderHeading = (text: string) => { page.drawText(text, { x: margin, y: y - 13, size: 13, font }); y -= 19; };
    ...
    console.error("image report error:", err);
